...
# Samlet højde af kilen
wedge_height <- height - 2 * 0.55

# 1 = 90 grader i spidsen
# >1 = spidsere
# <1 = stumpere
...
wedge_at <- function(frame) {
  ...
  tip_x <- -0.8 + (frame - 1) / (n_frames - 1) * (wedge_width + 3.4)
  tip_y <- height / 2
  ...
  for (iter in seq_len(constraint_iterations)) {
    ...
    for (j in 2:n_lines) {
      ...
      if (any(bad)) {
        ...
        vy_mat[j, bad] <-
          vy_mat[j, bad] + 0.25 * correction / dt
        
        vy_mat[j - 1, bad] <-
          vy_mat[j - 1, bad] - 0.25 * correction / dt
      }
    }
  }
...
for (f in seq_len(n_frames)) {
  ...
  if (material_contact) {
    ...
    # --------------------------------------------------------
    # Parallel del af kilen presser også materialet
    # --------------------------------------------------------
