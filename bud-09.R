...
# Hvor mange frames selve indtrængningen varer
# Lavere = hurtigere kile
...
wedge_at <- function(frame) {
  ...
  # Stopper når venstre bagkant rammer x = 0
...
project_equal_visible_areas <- function(
  state,
  wedge
) {
...
for (f in seq_len(n_frames)) {
  ...
  state <- project_equal_visible_areas(
    state,
    wedge
  )
